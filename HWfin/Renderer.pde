public class ShapeRenderer{
    ...
    public ShapeRenderer(){
      box = new Box(34, 50, width - 54, height - 100);
      box.setBoxColor(250);
...
public class PencilRenderer implements Renderer{
    ...
    public void render(){
        ...
        if(mousePressed){
            ...
            if(!once){
                once = true;
                shapeRenderer.addShape(new Point(points,penColor)); 
    ...
    public void render(){
        ...
        if(mousePressed&& mouseButton == LEFT){
            if(!once){
                ...
                if(first_click){
                    shapeRenderer.addShape(new Line(first_point,second_point,penColor));
                    first_point = null;
                    second_point = null;
                }
    ...
    public void render(){
        ...
        if(mousePressed&& mouseButton == LEFT){
              if(!once){
                  ...
                  if(first_click) second_point = new Vector3(mouseX,mouseY,0);             
                  if(first_click){
                      shapeRenderer.addShape(new Circle(first_point,distance(first_point,second_point),penColor));
                      first_point = null;
                      second_point = null;
                  }
                  first_click = !first_click;
...
public class PolygonRenderer implements Renderer{
    ...
    public void render(){
        ...
        if(mousePressed&& mouseButton == LEFT){
            ...
            if(!once){
              shapeRenderer.addShape(new Polygon(verties,penColor));
        ...
        if(mousePressed&& mouseButton == LEFT){
            if(!once){
                ...
                if(times==2){
                    ...
                    radius2 = dist;
                    shapeRenderer.addShape(new Ellipse(center,radius1,radius2,penColor));
                }
                times += 1;
                times %=3;
                once = true;
          ...
          if(!once){   
              switch(times){
                ...
                case 3:
                  ...
                  shapeRenderer.addShape(new Curve(cp1,cp3,cp4,cp2,penColor));
...
// 噴槍 Renderer：按住左鍵即噴，放開就固化為 Shape
public class SprayRenderer implements Renderer {
  ...
  @Override public void render(){
    ...
    // 預覽圈（GUI 行為允許用 rect/ellipse；不畫也可）
    noFill(); stroke(0);
    ...
    if(mousePressed && mouseButton == LEFT){
      once = false;
      // 即時噴點
      CGSpray(mouseX, mouseY, sprayRadius, sprayDensity);
      // 同步記錄點（用較稀疏的版本以免太大）
      for (int i=0; i<sprayDensity/2; i++){
        float a = random(TWO_PI), r = sprayRadius * sqrt(random(1));
        pts.add(new Vector3(mouseX + r*cos(a), mouseY + r*sin(a), 0));
      }
    } else {
      if(!once && pts.size()>0){
        shapeRenderer.addShape(new Spray(new ArrayList<Vector3>(pts), penColor));
        pts.clear();
        once = true;
      }
    }
...
// 箭頭 Renderer：滑鼠所在即時預覽；左鍵單擊固化一個箭頭
public class ArrowRenderer implements Renderer {
  boolean once = false;
  String dir;
  public ArrowRenderer(String d){ dir = d; }

  @Override public void render(){
    if(!shapeRenderer.checkInBox(new Vector3(mouseX,mouseY,0))) return;

    // 1) 即時預覽（半透明）
    CGArrow(mouseX, mouseY, arrowSize, dir, 180);

    // 2) 固化（只在按下那一瞬間做一次）
    if (mousePressed && mouseButton == LEFT) {
      if (!once) {
        shapeRenderer.addShape(new Arrow(new Vector3(mouseX,mouseY,0), arrowSize, dir, penColor));
        once = true;
      }
    } else {
      once = false;
